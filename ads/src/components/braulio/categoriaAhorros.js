...
import { getAhorrosPorCategoria } from "../../api/api2";
import {
  Chart as ChartJS,
  ArcElement,
  Tooltip,
...
const CategoriaAhorros = ({ fechaInicio, fechaFinal }) => {
  ...
  useEffect(() => {
    if (fechaInicio && fechaFinal) {
      const fetchData = async () => {
        try {
          const ahorros = await getAhorrosPorCategoria(fechaInicio, fechaFinal);
          setDatos(ahorros);
        } catch (error) {
          console.error("Error al obtener los datos de ahorros: ", error);
...
export default CategoriaAhorros;